...
(function() {
  ...
  DocPad = (function(_super) {
    ...
    DocPad.prototype.getTemplateData = function(userTemplateData) {
      var docpad, locale, templateData, _base, _base1, _base2;
      ...
      (_base = templateData.site).url || (_base.url = 'http://' + (docpad.getHostname() || 'localhost') + ':' + (docpad.getPort()));
      (_base1 = templateData.site).date || (_base1.date = new Date());
      (_base2 = templateData.site).keywords || (_base2.keywords = []);
    ...
    function DocPad(instanceConfig, next) {
      ...
      var action, configEventContext, docpad, logger, loggerConsole, loggers, methodName, _i, _len, _ref1, _ref2;
      _ref1 = extractOptsAndCallback(instanceConfig, next), instanceConfig = _ref1[0], next = _ref1[1];
      docpad = this;
      _ref2 = 'log error warn trackError checkRequest subscribe track identify serverMiddlewareRouter serverMiddlewareHeader serverMiddleware404 serverMiddleware500'.split(/\s+/);
      ...
      this.actionRunnerInstance = TaskGroup.create('action runner').whenDone(function(err) {
        if (err) {
          return docpad.error(err);
        }
      });
      this.errorRunnerInstance = TaskGroup.create('error runner').whenDone(function(err) {
        var locale, _ref3;
        if (err && docpad.getDebugging()) {
          locale = docpad.getLocale();
          return docpad.log('warn', locale.reportError + ' ' + locale.errorFollows + ' ' + ((_ref3 = err.stack) != null ? _ref3 : err.message).toString());
        }
      });
      this.trackRunnerInstance = TaskGroup.create('track runner').whenDone(function(err) {
        var locale, _ref3;
        if (err && docpad.getDebugging()) {
          locale = docpad.getLocale();
          return docpad.log('warn', locale.trackError + ' ' + locale.errorFollows + ' ' + ((_ref3 = err.stack) != null ? _ref3 : err.message).toString());
        }
      });
      ...
      this.database = new FilesCollection(null, {
        ...
        var outPath, updatedModels, url, _j, _len1, _ref3;
        ...
        var url, _j, _k, _len1, _len2, _ref3, _ref4;
        if (model.get('write') === false) {
          return;
        }
        _ref3 = model.previous('urls') || [];
        for (_j = 0, _len1 = _ref3.length; _j < _len1; _j++) {
          url = _ref3[_j];
          delete docpad.filesByUrl[url];
        }
        _ref4 = model.get('urls');
        ...
        if ((outPath = model.get('outPath'))) {
          existingModelId = (_base = docpad.filesByOutPath)[outPath] != null ? _base[outPath] : _base[outPath] = model.id;
    ...
    DocPad.prototype.error = function(err, type, next) {
      ...
      if (!err || err.logged) {
        ...
        err.logged = true;
        message = ((_ref1 = err.stack) != null ? _ref1 : err.message).toString().trim();
    ...
    DocPad.prototype.track = function(name, things, next) {
      var config, data, docpad, trackRunner, _ref1;
      if (things == null) {
        things = {};
      }
      docpad = this;
        ...
        if (docpad.userConfig.identified !== true) {
          ...
          docpad.getTrackRunner().addTask('create new user', function(complete) {
            return superAgent.post(config.helperUrl).type('json').set('Accept', 'application/json').query({
              method: 'analytics',
              action: 'identify'
            }).send(data).timeout(30 * 1000).end(docpad.checkRequest((function(_this) {
              return function(err) {
          ...
          docpad.getTrackRunner().addTask('update user', (function(_this) {
            return function(complete) {
              return superAgent.post(config.helperUrl).type('json').set('Accept', 'application/json').query({
                method: 'analytics',
                action: 'identify'
    ...
    DocPad.prototype.renderText = function(text, opts, next) {
      var attributes, clone, document, _ref1;
      _ref1 = extractOptsAndCallback(opts, next), opts = _ref1[0], next = _ref1[1];
