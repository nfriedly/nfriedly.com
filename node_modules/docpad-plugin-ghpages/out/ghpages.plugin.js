...
module.exports = function(BasePlugin) {
  ...
  return GhpagesPlugin = (function(_super) {
    ...
    GhpagesPlugin.prototype.deployToGithubPages = function(next) {
      var config, docpad, err, outGitPath, outPath, rootPath, _ref1, _ref2;
      docpad = this.docpad;
      ...
      return rimraf(outGitPath, function(err) {
        if (err) {
          return next(err);
        }
        docpad.log('debug', 'Performing static generation...');
        return docpad.generate(function(err) {
          if (err) {
            return next(err);
          }
          ...
          return safeps.spawnCommand('git', ['config', "remote." + config.deployRemote + ".url"], {
            ...
            return safeps.spawnCommand('git', ['log', '--oneline'], {
              ...
              var gitCommands, lastCommit, pushCommand;
              if (err) {
                return next(err);
              }
              ...
              if (config.quiet) {
                pushCommand.push('--quiet');
              }
              gitCommands = [['init'], ['add', '.'], ['commit', '-m', lastCommit], pushCommand];
